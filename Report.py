...
class Report:
    ...
    def compare_inventories(self, old_inventory, new_inventory) :
        ...
        if self.itemsDetail == [] :
            ...
            print(f'Comparison done : found {len(comparison)} item count change : {comparison}')
    ...
    def _get_trading_post_prices(self, item_id, ptr_lock, ptr_prices_table):
        ...
        priceResponse = gw2h.GW2_API_handler(url)
        ...
        if priceResponse == '': # No answer from API => Set output as ""
            ...
            if priceResponse["sells"] == []:
                raise ValueError (f'No sell offer for item {item_id}. Contacter Guigui: ça l intéresse !')
            else :
            ...
            with ptr_lock:
                ptr_prices_table[item_id] = {'sell' : lowestSellOffer,
                                         'buy' : highestBuyOffer}
    ...
    def _get_item_properties(self, item_id, ptr_lock, ptr_item_properties_table):
        ...
        propertyResponse = gw2h.GW2_API_handler(url)
    ...
    def get_item_details(self):  
        ...
        tpPrices = dict() # trading post prices. Key = item id, value = lowest sell offer & highest buy offer
        for i in range(0,len(self.itemsDetail)):
            # Get trading post prices for each item
            threads.append(threading.Thread(target=self._get_trading_post_prices,
                                            args=(self.itemsDetail[i]['id'],
                                            lock,
                                            tpPrices)))
            # Get property data for each item (especially name & NPC price)
            threads.append(threading.Thread(target=self._get_item_properties,
                                            args=(self.itemsDetail[i]['id'],
                                            lock,
                                            propertyResponse)))
        ...
        for i in range(0,len(self.itemsDetail)):
            ...
            if 'name' in self.itemsDetail[i].keys() :
                self.itemsDetail[i] = {'id' : self.itemsDetail[i]['id'],
                                             'count' : self.itemsDetail[i]['count']}
            # Initialize comment field
            self.itemsDetail[i]['comment'] = ""            
            
            if propertyResponse[self.itemsDetail[i]['id']] == "": # Unable to get properties (example : Augmentation au niveau 80 object : id = 78599)
                debug_id = self.itemsDetail[i]['id']
                print(f'Could not retrieve properties for item {debug_id} => Skipping value computation for this item.')
                self.itemsDetail[i]['name'] = "UNKOWN"
                self.itemsDetail[i]["aquisition price"] = 0
                self.itemsDetail[i]['liquid gold value'] = 0
                self.itemsDetail[i]['comment'] = self.itemsDetail[i]['comment'] + "Unkown item. "
            else :
                self.itemsDetail[i]['name'] = propertyResponse[self.itemsDetail[i]['id']]['name']
                urllib.request.urlretrieve(propertyResponse[self.itemsDetail[i]['id']]['icon'], "Download/" + str(self.itemsDetail[i]['id']) + ".png")
                
                # NoSell : item is not sellable to NPC => Price = 0
                if 'NoSell' in propertyResponse[self.itemsDetail[i]['id']]['flags']:
                    npcPrice = 0
                    self.itemsDetail[i]['comment'] = self.itemsDetail[i]['comment'] + "Item not sellable to NPC. "
                else :
                    npcPrice = propertyResponse[self.itemsDetail[i]['id']]['vendor_value'] 
                    
                # Compute aquisition price
                if tpPrices[self.itemsDetail[i]['id']] == "" : # Item is not sold on trading post
                    self.itemsDetail[i]['aquisition price'] = 0
                    self.itemsDetail[i]['liquid gold value'] = npcPrice
                    self.itemsDetail[i]['comment'] = self.itemsDetail[i]['comment'] + "Item not sold on trading post. "
                else :
                    self.itemsDetail[i]["aquisition price"] = tpPrices[self.itemsDetail[i]['id']]['sell']
                    
                    if tpPrices[self.itemsDetail[i]['id']]['buy'] == [] :
                        self.itemsDetail[i]["liquid gold value"] = npcPrice
                        self.itemsDetail[i]['comment'] = self.itemsDetail[i]['comment'] + "No buyer on trading post. "
                    else :
                        self.itemsDetail[i]["liquid gold value"] = max(npcPrice, 
                                                                       round(0.85*tpPrices[self.itemsDetail[i]['id']]['sell']))
                    
                    if npcPrice >= 0.85*tpPrices[self.itemsDetail[i]['id']]['sell'] :
                        self.itemsDetail[i]['comment'] = self.itemsDetail[i]['comment'] + "TP price low : sell to NPC instead. "
